...
screen.file.results="~/Work/WCMC/Studer/mDA_vivo_survival/TNF-NFKB-p53-axis-restricts-in-vivo-survival-of-hPSC-derived-dopamine-neuron-/CRISPR-screen-processing/internal-data/220929\ large\ pool\ gRNA\ raw\ and\ processed.xlsx"
large.screen.data = list(
  D16=read.xlsx(screen.file.results, sheet=1, rows=1:539),
  D25m=read.xlsx(screen.file.results, sheet=2, rows=1:539),
  D25p=read.xlsx(screen.file.results, sheet=3, rows=1:539),
  D25g=read.xlsx(screen.file.results, sheet=4, rows=1:539)
)
```
...
assertthat::assert_that(
  all.equal(large.screen.data$D16$NAME, large.screen.data$D25m$NAME),# [1] TRUE
  all.equal(large.screen.data$D16$NAME, large.screen.data$D25p$NAME),# [1] TRUE
  all.equal(large.screen.data$D16$NAME, large.screen.data$D25g$NAME),# [1] TRUE
  length(setdiff(large.screen.data$D16$NAME, large.screen.data$D25g$NAME)) == 0, # character(0)
  msg = "D16 and D25 are not he same gene sets"
  )
...
Figure 1C
[TODO : Why are we loading in the same files multiple times?]
...
#   D25m=fread("1e.2.txt"),
#   D25p=fread("1e.3.txt"),
#   D25g1=fread("1e.4.txt"),
#   D25g2=fread("1e.5.txt")
# )
